...
Description: Наиболее полный и простой в использовании плагин членства, готовый разрешить или ограничить ваше содержание страницы для определенных пользователей.
Version: 6.0
Author: indeed
...
function ihc_initiate_plugin(){
	...
	if (defined('PHP_VERSION') && version_compare(PHP_VERSION, 5.3, '<')){
		...
		die('Ultimate Pro членство требует PHP версии больше 5.3, вашей текущей PHP v.' . PHP_VERSION . ' . Обновите ваш PHP и попробуйте еще раз!');
	} 
	...
	if ($first_before_expire){
		...
		if ($u_ids){
			foreach ($u_ids as $u_data){
				...
				ihc_send_user_notifications($uid, 'admin_before_user_expire_level', $u_data->level_id);/// SEND NOTIFICATION TO ADMIN				
				if ($sent){
