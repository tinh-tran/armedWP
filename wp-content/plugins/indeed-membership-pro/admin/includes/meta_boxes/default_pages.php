...
if ($set_arr && count($set_arr) && in_array($post->ID, $set_arr)) {
	...
	if ($unset_arr){		
		...
		?>
		<div class="ihc-meta-box-message" style="color: #333;"><?php echo __('All the require pages are set properly, to change them click ', 'ihc') . '<a href="' . admin_url('admin.php?page=ihc_manage&tab=general') . '">here</a>.';?></div>
		<?php	
	}
}