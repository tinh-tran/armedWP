...
if(!function_exists('ult_info_table_generate_design05')) {
	function ult_info_table_generate_design05($atts,$content = null){
		...
		if($use_cta_btn == "box"){
			$output .= '<a href="'.$link.'" '.$target.' '. $link_title .' '. $rel .' class="ult_price_action_button" style="'.$featured_style.'">'.$package_btn_text;
		}
		...
		$output .= '<div class="ult_pricing_table_wrap ult_info_table ult_design_5 '.$featured.' ult-cs-'.$color_scheme.' '.$el_class.' '.$css_info_tables.'">
			...
			if($use_cta_btn == "true"){
				$output .= '<div id="'.$info_table_btn_id.'"  class="ult_price_link" style="'.$normal_style.'">
							<a href="'.$link.'" '.$target.' '. $link_title .' '. $rel .'  '.$info_table_btn_data_list.' class="ult-responsive ult_price_action_button" style="'.$featured_style.' '.$button_inline.'">'.$package_btn_text.'</a>
						</div><!--ult_price_link-->';
			}
